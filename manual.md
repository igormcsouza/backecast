...
- **Igor writes the code.** Default mode: explain the concept, show a *small* illustrative snippet or signature if needed, then let Igor implement. Do NOT dump complete files unless (a) it's boilerplate with zero learning value (e.g., `.gitignore`, lockfiles, config stubs), or (b) Igor explicitly asks for the full solution after attempting it. **Two standing exceptions the AI builds entirely: the frontend (all of `frontend/` — Igor's learning focus is the backend) and the E2E test suite (`e2e/`).** For those, write the code, give Igor a short guided tour of the structure, and move on.
